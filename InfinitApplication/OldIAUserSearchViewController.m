//
//  OldIASearchResultsViewController.m
//  InfinitApplication
//
...
#import "OldInfinitTokenAttachmentCell.h"

//- Search View Element ----------------------------------------------------------------------------

@implementation OldInfinitSearchElement
...
{
  if ([object isKindOfClass:OldInfinitSearchElement.class])
  {
...
@implementation OldIASearchBoxView
...
@implementation OldIAUserSearchViewController
{
  // WORKAROUND: 10.7 doesn't allow weak references to certain classes (like NSViewController)
  __unsafe_unretained id<OldIAUserSearchViewProtocol> _delegate;
  ...
  OldInfinitSearchController* _search_controller;
  ...
  if (self = [super initWithNibName:self.className bundle:nil])
  {
    ...
    _add_file_str = [[NSAttributedString alloc] initWithString:
                     NSLocalizedString(@"Don't forget to add a file.", nil)
                                                    attributes:no_result_msg_style];
    _static_image = [IAFunctions imageNamed:@"old-icon-search"];
    
    _search_controller = [[OldInfinitSearchController alloc] initWithDelegate:self];
    _last_search = @"";
...
- (void)addUser:(IAUser*)user
{
  ...
  OldInfinitSearchElement* element;
  for (OldInfinitSearchElement* other in _search_results)
  {
...
- (void)removeUser:(IAUser*)user
{
  ...
  for (OldInfinitSearchElement* element in recipients)
  {
...
- (void)addElement:(OldInfinitSearchElement*)element
{
  if (element == nil)
    return;
...
doCommandBySelector:(SEL)commandSelector
{
  ...
  if (commandSelector == @selector(deleteBackward:))
  {
    ...
    if ([obj isKindOfClass:OldInfinitSearchElement.class])
    {
  ...
  else if (commandSelector == @selector(insertNewline:))
  {
    ...
    {
      OldInfinitSearchElement* element = _search_results[row];
    ...
    else if (_search_results.count == 0)
    {
      ...
      {
        OldInfinitSearchElement* element =
          [[OldInfinitSearchElement alloc] initWithAvatar:[IAFunctions makeAvatarFor:@"@"]
                                                    email:search_string
...
{
  if ([representedObject isKindOfClass:OldInfinitSearchElement.class])
  {
...
- (void)fillSearchResultsWithFriends
{
  ...
  for (IAUser* user in _favourites_swaggers)
  {
    ...
    {
      OldInfinitSearchElement* element =
        [[OldInfinitSearchElement alloc] initWithAvatar:[IAAvatarManager getAvatarForUser:user]
                                                  email:nil
...
- (void)updateResultsTable
{
  ...
  for (OldInfinitSearchElement* element in self.search_field.objectValue)
  {
                 ...
                 row:(NSInteger)row
{
  ...
  OldIASearchResultsCellView* cell;
...
- (void)searchControllerGotEmailResult:(OldInfinitSearchController*)sender
{
  ...
  else
  {
    ...
    OldInfinitSearchElement* element =
      [[OldInfinitSearchElement alloc] initWithAvatar:person.avatar
                                                email:nil
...
{
  NSInteger index = 0;
  for (OldInfinitSearchElement* element in _search_results)
  {
...
- (void)searchResultCellWantsAddFavourite:(OldIASearchResultsCellView*)sender
{
  ...
  OldInfinitSearchElement* element = [_search_results objectAtIndex:row];
  if (element.user == nil)
    return;
 ...
 attachmentCellForRepresentedObject:(id)representedObject
{
  ...
  OldInfinitTokenAttachmentCell* cell = [OldInfinitTokenAttachmentCell new];
