//
//  OldInfinitSendNoteViewController.m
//  InfinitApplication
//
...
@implementation OldInfinitSendNoteField
{
@private
...
{
  [(id<OldInfinitSendNoteProtocol>)_delegate gotFocus:self];
...
- (NSSize)intrinsicContentSize
{
  ...
  {
    [(id<OldInfinitSendNoteProtocol>)_delegate changedHeightBy:(height - _last_height)];
    _last_height = height;
  }
...
@interface OldInfinitSendNoteView : NSView
@property (nonatomic, readwrite) BOOL link_mode;
@end

@implementation OldInfinitSendNoteView
...
@interface OldInfinitSendNoteViewController ()
@end

@implementation OldInfinitSendNoteViewController
{
  // WORKAROUND: 10.7 doesn't allow weak references to certain classes (like NSViewController)
  __unsafe_unretained id<OldInfinitSendNoteViewProtocol> _delegate;
...
{
  _link_mode = link_mode;
  ((OldInfinitSendNoteView*)self.view).link_mode = link_mode;
}
