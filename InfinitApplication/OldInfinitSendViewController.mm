//
//  OldInfinitSendViewController.m
//  InfinitApplication
//
...
@implementation OldInfinitSendUserLinkView
{
@private
  // WORKAROUND: 10.7 doesn't allow weak references to certain classes (like NSViewController)
  __unsafe_unretained id<OldInfinitSendUserLinkProtocol> _delegate;
...
@implementation OldInfinitSendDropView
{
  NSArray* _drag_types;
}
...
@interface OldInfinitSendViewController ()
@end

@implementation OldInfinitSendViewController
{
@private
  __weak id<InfinitSendViewProtocol> _delegate;

  // WORKAROUND: 10.7 doesn't allow weak references to certain classes (like NSViewController)
  OldIAUserSearchViewController* _search_controller;
  OldInfinitSendNoteViewController* _note_controller;
  OldInfinitSendFilesViewController* _files_controller;
               ...
               forLink:(BOOL)for_link;
{
  if (self = [super initWithNibName:self.className bundle:nil])
  {
    ...
    _note_controller = [[OldInfinitSendNoteViewController alloc] initWithDelegate:self];
    _files_controller = [[OldInfinitSendFilesViewController alloc] initWithDelegate:self];
...
- (void)doSend
{
  if (self.user_link_view.mode == INFINIT_USER_MODE)
  {
    ...
    {
      if ([element isKindOfClass:OldInfinitSearchElement.class])
      {
        if ([element user] == nil)
