...
@implementation OldInfinitSelectedBoxView
...
{
  if (check_mark == nil)
    check_mark = [IAFunctions imageNamed:@"old-icon-contact-checked"];
...
@implementation OldIASearchResultsCellView
{
  BOOL _is_favourite;
  BOOL _infinit_user;
  // WORKAROUND: 10.7 doesn't allow weak references to certain classes (like NSViewController)
  __unsafe_unretained id<OldIASearchResultsCellProtocol> _delegate;
...
- (void)setUserFavourite:(BOOL)favourite
{
  ...
  if (favourite)
  {
    self.result_star.image = [IAFunctions imageNamed:@"old-icon-star-selected"];
  ...
  {
    self.result_star.image = [IAFunctions imageNamed:@"old-icon-star"];
